...
**已实现功能**：用户注册/登录/退出、菜品浏览与搜索、购物车与下单、订单查看；管理员登录、分类/菜品/订单/用户 CRUD、数据看板（4 类分析图表）；JWT 认证 + 角色鉴权（路由守卫 + API 拦截器）。
...
导入测试数据后可用以下账号登录（密码均为 `123456`）：

| 账号 | 角色 | 登录入口 |
|------|------|----------|
| `user001` ~ `user200` | 普通用户 | `/login`（跳转首页） |
| `admin1` ~ `admin5` | 管理员 | `/admin/login`（跳转管理后台） |

**权限说明**：
- 未登录用户可浏览菜品，购物车和订单需登录
- 普通用户不能访问 `/admin/*` 路由（路由守卫拦截）
- 首页"经营看板"按钮仅管理员可见
