...
| U-02 | 用户登录 | 登录后返回 token + 用户信息，跳转首页；管理员从管理端登录页登录 |
...
- [x] 完善 pom.xml，引入 SpringBoot、MyBatis Plus、Redis、JWT 等依赖
- [x] 配置文件（application.yml、.env.example）
- [x] 数据库建表 SQL 脚本
- [x] 基础包结构创建
- [x] 统一响应封装、全局异常处理
- [x] JWT 登录认证 + 拦截器
...
- [x] Vue 项目初始化
- [x] 用户端页面（登录、首页、菜品详情、购物车、订单）
- [x] 管理端页面（工作台、分类、菜品、订单、用户管理）
- [x] 数据可视化页面（客流、高峰、销量、预测）
